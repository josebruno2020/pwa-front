<template>
  <main>
    <div id="print-nurse">
      <h2 class="report-title">Relatório de Evolução de Enfermagem</h2>
        ...
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            ...
            <span class="report-action no-print"  v-if="report.user.id === userId">
              ...
              <el-button size="mini" type="danger" plain @click="deleteReport(report.id)">
...
@Component({
  name: 'HistoryNurseEvolutionReport'
})
export default class HistoryNurseEvolutionReport extends Mixins<LoadingMixin>(LoadingMixin, DateMixin, EditRowMixin) {
  loading = false
  ...
  async fetchReport() {
    ...
    this.reports = []
    try {
      const {data} = await httpGet(`${apiRoutes.nurseEvolution}/${this.patient.id}`)
  ...
  async updateReport(reportId: number, newReport: string) {
    ...
    try {
      await httpPut(`${apiRoutes.nurseEvolution}/${reportId}`, {
        report: newReport
      });
      this.$notify.success({
        title: 'Sucesso',
  ...
  async deleteReport(reportId: number) {
    this.openFullScreenLoading()
    try {
      await httpDelete(`${apiRoutes.nurseEvolution}/${reportId}`);
      this.$notify.success({
        title: 'Sucesso',
