<template>
  <main>
    <div id="print-nurse">
      <h2 class="report-title">Relatório de Enfermagem</h2>
...
export default class HistoryNurseReport extends Mixins<LoadingMixin>(LoadingMixin, DateMixin, EditRowMixin, PrintMixin) {
  ...
  async fetchReport() {
    ...
    this.reports = []
    try {
      const {data} = await httpGet(`${apiRoutes.nurseReport}/${this.patient.id}`)
  ...
  async updateReport(reportId: number, newReport: string) {
    ...
    try {
      await httpPut(`${apiRoutes.nurseReport}/${reportId}`, {
        report: newReport
      });
      this.$notify.success({
        title: 'Sucesso',
  ...
  async deleteReport(reportId: number) {
    this.openFullScreenLoading()
    try {
      await httpDelete(`${apiRoutes.nurseReport}/${reportId}`);
      this.$notify.success({
        title: 'Sucesso',
